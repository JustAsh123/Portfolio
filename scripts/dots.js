...
class Dot {
  constructor() {
    ...
    // Direction vector from center to dot
    const dx = this.x - centerX;
    const dy = this.y - centerY;

    const length = Math.sqrt(dx * dx + dy * dy) || 1; // avoid division by 0

    // Normalize and scale velocity
    const speed = 0.2 + Math.random() * 0.2; // speed between 0.6 and 1.2
...
function animate() {
  ...
  if (mouse.x && mouse.y) {
    const nearbyDots = dots.filter(
      (dot) => dot.distanceSqTo(mouse.x, mouse.y) <= MOUSE_RADIUS * MOUSE_RADIUS
    );
